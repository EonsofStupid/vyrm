# RRFlow / SurrealDB claim-runtime differential
...
RRFlow runs embedded in an isolated benchmark child. SurrealDB runs as a local
...
RRFlow RSS includes its small Rust harness; that asymmetry favors Surreal and is
...
both systems passed full verification. RRFlow measured:

| Cell | RRFlow / SurrealDB | Interpretation |
|---|---:|---|
| Write throughput, end-to-end | 57.08× | RRFlow higher |
| Read throughput, end-to-end | 895.61× | RRFlow higher |
| Write throughput, Surreal server time | 10.19× | RRFlow higher |
| Read throughput, Surreal server time | 17.50× | RRFlow higher |
| Write p95, end-to-end | 0.031× | RRFlow lower |
| Read p95, end-to-end | 0.001× | RRFlow lower |
| Clean-start readiness | 0.005× | RRFlow lower |
| Reopened process RSS | 0.060× | RRFlow lower |
| Reopened allocated bytes | 1.380× | SurrealDB lower |

The bounded verdict is therefore **not** “all measured cells favor RRFlow.” The
disk result is a useful lifecycle finding: this RRFlow profile reopens from its
authoritative WAL without forced maintenance, while its explicitly maintained
segment is much smaller. The comparison must not substitute RRFlow's maintained
...
corpus that crosses RRFlow's maintenance threshold, equivalent maintained-state
...
Evidence:
[`2026-08-23-rrflow-surrealdb-3.0.5-claim-diagnostic-v1.json`](../eval/results/2026-08-23-rrflow-surrealdb-3.0.5-claim-diagnostic-v1.json).
