...
| Vector and full-text retrieval | Revisioned dense/sparse/multi-dense collections, ACID batch point upsert/retirement, exact retrieve/scroll/search, typed payload-index lifecycle, guarded collection deletion, online filtered HNSW with authoritative deltas and incremental immutable generations, content-addressed BM25 v2, persistent TurboQuant artifacts, and a CPU-oracle-qualified optional GPU HNSW build boundary share one engine authority. One-stamp recursive retrieval owns keyword/dense/sparse RRF, recommend/discover/context, multimodal/model MaxSim reranking, boost, MMR, groups, facets, and directed matrices. | Complete sparse inverted indexing, DBSF/general formulas, remaining codec/tier lifecycles, generated administration/query bindings, compact graph/payload-index layouts, production GPU adapters, and fixed-hardware evidence |
...
**Status:** in progress. The engine foundation persists collection and
...
pinned/cached/cold policy. Collection-addressed exact search enforces the
definition. Atomic vector writes bind the same
collection/name and validate field, kind, dimensions, and model provenance
before committing payload properties with the unified transaction. That same
transaction is the batch upsert/delete authority through `put_vector` and
valid-time `retire_data`; no parallel point store or write coordinator exists.
Typed payload-index ensure/list/delete transitions have distinct authorization,
generation, digest, CAS, journal, idempotency, and reopen semantics. Indexed
payload values are type-checked before the entire point batch commits.
...
- Collections/points/named-vector/payload administration. Collection and
  named-vector ensure/list/delete, collection-bound atomic batch vector/payload
...
- Unified nearest/recommend/discover/context/group/facet/directed-matrix,
  nested-prefetch, dense+sparse+keyword RRF, governed boost/exact/model/MMR, and
  ColBERT MaxSim now execute in one engine algebra without middleware. Remaining
...
- Per-named-vector pinned/cached/cold policy is physically enforced by one
  ...
  remain open.
- GPU HNSW construction qualification is implemented: public CPU/prefer/require
  policy, one process-local engine registry, CPU byte and semantic differential,
  deterministic fallback, atomic durable build/resource evidence, restart-safe
  replay, and public search work accounting pass. Production CUDA/ROCm/Metal/
  Vulkan adapters and fixed-hardware certification remain deployment work.
