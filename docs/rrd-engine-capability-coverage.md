...
| HNSW vector search with cosine, dot, Euclidean, Manhattan, and SIMD | Collection-bound HNSW v2 builds, incrementally advances, publishes, reopens, traverses, and exactly reranks all four metrics. Construction is portable scalar; query traversal runtime-dispatches AVX2. A fixed 512-vector corpus proves scalar/automatic identity and at least 0.95 mean Recall@10 for every metric at 100/50/10/1% selectivity. | **Engine core qualified.** Compact graph layout, fixed-hardware production latency/throughput, distributed placement, and GPU construction remain G04-W04/G04-W07. | G04-W02, G04-W04, G04-W07 |
...
| Multiple vectors, multimodal retrieval, and ColBERT-style late interaction | Revisioned collections persist named dense, sparse, and multi-dense definitions. Point batches upsert/retire through the shared ACID transaction log; exact retrieve/scroll survive reopen. Typed payload-index ensure/list/delete and fail-closed empty/artifact-free collection deletion are engine operations with distinct authorization actions. Multi-dense search uses MaxSim. | **Collection/point core present.** Generated administration bindings belong to G06; end-to-end multimodal ingestion/model provenance and the complete late-interaction surface remain G04-W03/G04-W06 work. | G04-W01, G04-W03, G04-W06 |
