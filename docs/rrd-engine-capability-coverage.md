...
|---|---|---|---|
| Native document, graph, relational, time-series, geo, key-value, strict-schema, and schemaless models | One revisioned `RuntimeSchemaRegistry` names its namespace/database and all logical models. One runtime commit can create/update/retire/recreate document, key-value, record, relation/native-edge, event, vector, series, geo, and object values; one exact-stamp snapshot reduces them under the same catalogue revision and authenticated cursor. Mixed-model atomicity, historical reads, concurrent-writer fencing, and Memory/Fjall/native reopen parity are covered. | **Engine CRUD core present as the G03-W02 candidate.** Mutating RRFlowQL and dedicated HTTP/MCP administration surfaces remain open under G03-W03/G06; reasoning claims deliberately retain their separate bi-temporal correction contract. | G03-W01, G03-W02, G03-W03, G06-W02 |
...
| Multi-row, multi-table ACID transactions | One `RuntimeCommit` atomically carries mixed mutation families under one global cursor; the server stages, previews, commits, aborts, and idempotently replays client transactions. G03-W02 adds pre-commit retirement validation at the observed read cursor and proves failed mixed-model batches publish nothing. | **Core present; deeper fault qualification open.** Same-cursor concurrent writers are fenced and Fjall/native reopen is exact; the complete crash/disk-full matrix remains G05-W02. | G02-W01, G03-W02, G05-W02 |
