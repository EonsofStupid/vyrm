...
| RRFlowQL, GraphQL, REST/HTTP, WebSocket, gRPC, MCP, CLI, SDKs, and Connectome | One validated `ProductCapabilityCatalogue` emits a total, canonically ordered ten-surface disposition matrix for every HTTP endpoint, WebSocket stream, runtime tool, work-plan command, and planned foundation operation. HTTP and WebSocket server routing resolves from `EndpointCatalogue`; MCP, CLI, SDK, and Connectome entrypoints merge from their executable registries. Five checked-in language endpoint maps and the TypeScript schema carry the exact OpenAPI digest, and dependency-light CI compares all 34 HTTP operation identities/methods/paths/authentication/mutation flags with the contract export. MCP additionally projects every advertised tool into an ordered task-domain catalogue derived from canonical security actions; unfinished ingest, semantic-code-search, and security-administration rows are planned and advertise nothing. Embedded and daemon calls both enter `RrdEngine::invoke_runtime_tool` for policy and structured audit. | **Generated catalogue, task-level MCP, and complete CLI runtime foundation present.** Every runtime tool has a generated CLI entrypoint with strict bound-engine or authenticated-daemon selection; offline recovery remains separately typed and exclusive. RRFlowQL, REST/HTTP, the durable WebSocket stream, MCP/CLI runtime operations, the Rust client, and current generated language skeletons are represented honestly. GraphQL and gRPC remain explicitly unavailable; shared black-box Rust/TypeScript/Python/Go/Java/.NET SDK qualification remains G06-W05. | G06-W01, G06-W02, G06-W04, G06-W05 |
