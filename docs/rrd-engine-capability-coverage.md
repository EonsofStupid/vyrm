...
|---|---|---|---|
| Native document, graph, relational, time-series, geo, key-value, strict-schema, and schemaless models | One revisioned `RuntimeSchemaRegistry` now names its namespace/database and catalogues document, relational, graph-node/relation, key-value, vector, event, time-series, geo, object, reasoning, and lifecycle tables with explicit strict/schemaless mode. Unified commits enforce model-family admission for every non-claim mutation; cross-model migration rollback and reopen parity pass on all engines. | **Catalogue core present; CRUD completion open.** Canonical records already carry document/key-value identities selected by the catalogue, but dedicated document/key-value CRUD vocabulary, retirement, and complete surface parity remain G03-W02. | G03-W01, G03-W02 |
