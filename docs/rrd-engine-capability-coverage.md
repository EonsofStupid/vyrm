...
| Multi-row, multi-table ACID transactions | One `RuntimeCommit` atomically carries mixed mutation families under one global cursor; the server stages, previews, commits, aborts, and idempotently replays client transactions. G03-W03 adds bounded `BEGIN; MUTATE $binding; ...; COMMIT|CANCEL` programs that delegate to the same coordinator and prove commit, cancel, failed-mutation atomicity, retry, policy denial, and native reopen. | **Core present; public adapters and deeper fault qualification open.** Same-cursor concurrent writers are fenced and Fjall/native reopen is exact; the complete crash/disk-full matrix remains G05-W02. | G02-W01, G03-W02, G03-W03, G05-W02 |
