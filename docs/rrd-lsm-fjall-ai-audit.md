# Fjall → RRD LSM AI-runtime audit
...
Fjall is a compatibility oracle and engineering reference, not RRFlow's physical
backend. `rrd-lsm` already owns its WAL, MVCC sequence allocation, memtable,
...
RRD LSM and old Fjall directories move only through the verified migration path.

That does **not** mean the native engine is mature enough to dismiss Fjall.
Fjall is substantially ahead in general-purpose LSM mechanics. RRFlow earns a
...
`RRFlowQL` is RRFlow's language; `RRD LSM` is its storage engine. Calling the latter
...
These are primary project sources. No Fjall source was copied into RRFlow.

## What Fjall gets right

| Mechanism | Fjall strength | RRFlow response |
|---|---|---|
...
| Read amplification | Bloom/filter policies, partitioned filters and indexes, configurable pinning, block sizing, and restart intervals | Add only after negative-hit and fan-out profiles establish the policy per RRFlow data family |
| Physical isolation | A separate physical LSM per keyspace with cross-keyspace journal/transaction semantics | Measure against RRFlow's single ordered space: it is cheaper for one atomic causal commit but can cause family-level compaction/cache interference |
| Large values | Optional KV separation with compaction-time blob rewriting | Keep large immutable objects and vector artifacts in RRD storage coordinator/content-addressed storage; avoid duplicating a blob log inside RRD LSM |
...
RRFlow can specialize because its workload is not an arbitrary `BTreeMap`:
...
| Large artifacts | Content-addressed, immutable, verified, often streamed | RRD storage coordinator local/S3 object references; never amplify them through the WAL/LSM |
...
cargo run --release --locked -p rrd-store --example ai_hotset_benchmark -- \
  ...
  --output target/rrd-lsm-ai-metadata-fanout.json --require-promotion
```
...
- [`metadata fan-out, structured JSON`](../eval/results/2026-08-22-rrd-lsm-ai-metadata-fanout-structured-json.json)
- [`metadata fan-out, deterministic entropy`](../eval/results/2026-08-22-rrd-lsm-ai-metadata-fanout-deterministic-entropy.json)
- [`metadata fan-out, embedding f32`](../eval/results/2026-08-22-rrd-lsm-ai-metadata-fanout-embedding-f32.json)
...
[`standard streaming-scan v4`](../eval/results/2026-08-23-rrd-lsm-standard-streaming-scan-v4.json).
...
footprint. Evidence:
[`embedding batch-v2`](../eval/results/2026-08-23-rrd-lsm-ai-embedding-batch-v2.json).
