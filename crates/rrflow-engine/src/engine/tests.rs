use super::{public_runtime_commit, RrflowEngine, ServiceError};
use rrd_contract::{
    AbortTransaction, BeginTransaction, CanonicalId, CloseSession, CommitTransaction,
    CorrelationId, CreateSession, EnsureVectorCollection, NamedVectorDefinition, RenewSession,
    RequestContext, ResourceId, ResourceKind, ResourcePath, SecurityAction, SessionLimits,
    TransactionMutation, TransactionState, VectorMemoryTier, VectorSearchMetric, VectorValueKind,
};
...
fn isolated_engine() -> (tempfile::TempDir, RrflowEngine) {
    let root = tempfile::tempdir().unwrap();
    let engine = RrflowEngine::open(root.path(), instance(), TOKEN_KEY).unwrap();
    (root, engine)
}
