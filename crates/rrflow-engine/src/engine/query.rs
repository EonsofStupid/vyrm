...
impl RrflowEngine {
    ...
    pub fn execute_query(
        ...
        let catalog = vyrm_mx::Catalog::capture(&self.storage, &scope)
            .map_err(|error| ServiceError::Query(error.to_string()))?;
        ...
        let execution = vyrm_mx::execute(&self.storage, &plan, &budget)
            .map_err(|error| ServiceError::Query(error.to_string()))?;
        let rows = execution
            .batches
            .iter()
    ...
    pub fn ensure_query_index(
        ...
        let repository = vyrm_mx::IndexCatalogueRepository::new(&self.storage, scope.clone());
        ...
        if let Some(existing) = current.entries.get(&definition.id) {
            ...
            let query_catalogue = vyrm_mx::Catalog::capture(&self.storage, &scope)
                .map_err(|error| ServiceError::Query(error.to_string()))?;
            repository
    ...
    pub fn list_query_indexes(
        ...
        let scope = self.query_scope(&request.scope)?;
        let catalogue = vyrm_mx::IndexCatalogueRepository::new(&self.storage, scope)
            .load()
            .map_err(|error| ServiceError::Query(error.to_string()))?;
