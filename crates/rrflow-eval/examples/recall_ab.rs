...
//!   models what an agent without rrflow injects: every journal section that
...
const QUERIES: &[(&str, &[&str])] = &[
    ...
    ("routing projection", &["rrd-graph", "step-r"]),
...
fn main() {
    ...
    for (label, subjects) in QUERIES {
        ...
        if real > 0 {
            estimate_errors.push((set.token_estimate as f64 - real as f64).abs() / real as f64);
        }
