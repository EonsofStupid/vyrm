...
fn digest(claims: &[Claim]) -> String {
    let mut bytes = b"rrflow-recall-v1\0".to_vec();
...
mod tests {
    ...
    fn recalls_only_current_claims_of_the_subject_set() {
        ...
        let facts: Vec<_> = set
            .claims
            .iter()
            .map(|c| (c.predicate.as_str(), c.object.as_str()))
            .collect();
        assert_eq!(
            facts,
    ...
    fn resolution_is_as_of_not_latest() {
        ...
        let status = set
            .claims
            .iter()
            .find(|c| c.predicate.as_str() == "status")
            .unwrap();
        assert_eq!(
            status.object, "authored",
            "at 149 the superseded version was in force"
        );
        assert!(
        ...
        assert!(
            !at_boundary
                .claims
                .iter()
                .any(|c| c.predicate.as_str() == "blocker"),
    ...
    fn budget_truncates_and_says_so_but_never_returns_empty() {
        ...
        assert_eq!(
            set.claims.len(),
            1,
            "the first claim is included even over budget"
        );
    ...
    fn digest_identifies_content_not_invocation() {
        ...
        assert_eq!(
            a.digest, b.digest,
            "a repeated subject must not change the content"
        );
        let c = recall(&corpus(), &query(&["wp3", "wp9"], 250), 10_000).unwrap();
        assert_ne!(
            a.digest, c.digest,
            "different knowledge must not share a digest"
        );
    }
    ...
    fn digest_changes_when_recalled_provenance_changes() {
        ...
        assert_ne!(
            a.digest, b.digest,
            "provenance is part of recalled knowledge"
        );
    }
