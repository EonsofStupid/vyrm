    ...
    pub fn append(
        ...
        if self.events.last().is_some_and(|event| at < event.at) {
            return Err(ContractError(
                "reasoning event time must be monotonic".into(),
            ));
        }
        ...
        if let ReasoningPayload::Verification { checks } = &event.payload {
            self.last_verification_passed = Some(
                checks
                    .iter()
...
fn transition(
    ...
    let next = match (state, payload) {
        ...
        (
            S::NeedsDecision,
            P::Decision {
                decision: DecisionKind::Verify | DecisionKind::Stop,
                ..
            },
        ) => S::NeedsVerification,
        ...
        (S::NeedsOutcome, P::Outcome { outcome, .. }) => {
            match (last_verification_passed, outcome) {
                ...
                _ => {
                    return Err(ContractError(
                        "outcome contradicts verification evidence".into(),
                    ))
                }
            }
            S::Complete
...
fn validate_evidence(evidence: &Evidence) -> Result<(), ContractError> {
    ...
    {
        return Err(ContractError(
            "evidence.digest must be a SHA-256 hex digest".into(),
        ));
    }
    Ok(())
}
    ...
    match payload {
        P::Goal {
            ...
            if acceptance.is_empty() {
                return Err(ContractError("goal.acceptance must not be empty".into()));
            }
            for value in acceptance {
                validate_text("goal.acceptance", value)?;
            }
        }
        P::Plan { hypothesis, steps } => {
            validate_text("plan.hypothesis", hypothesis)?;
            if steps.is_empty() {
                return Err(ContractError("plan.steps must not be empty".into()));
            }
            for value in steps {
                validate_text("plan.steps", value)?;
            }
        }
        P::Attempt { summary, actions } => {
            validate_text("attempt.summary", summary)?;
            for value in actions {
                validate_text("attempt.actions", value)?;
            }
        }
        P::Observation { summary, evidence } => {
            ...
            if evidence.is_empty() {
                return Err(ContractError(
                    "observation.evidence must not be empty".into(),
                ));
            }
        ...
        P::Verification { checks } => {
            if checks.is_empty() {
                return Err(ContractError(
                    "verification.checks must not be empty".into(),
                ));
            }
            for check in checks {
...
fn event_digest(
    ...
    let mut bytes = b"rrflow-reasoning-event-v1\0".to_vec();
    ...
    match payload {
        ReasoningPayload::Goal {
            ...
            bytes.push(0);
            text(&mut bytes, statement);
            strings(&mut bytes, acceptance);
        }
        ReasoningPayload::Plan { hypothesis, steps } => {
            bytes.push(1);
            text(&mut bytes, hypothesis);
            strings(&mut bytes, steps);
        }
        ReasoningPayload::Attempt { summary, actions } => {
            bytes.push(2);
            text(&mut bytes, summary);
            strings(&mut bytes, actions);
        }
        ...
        ReasoningPayload::Decision {
            ...
            text(&mut bytes, rationale);
        }
        ReasoningPayload::Verification { checks } => {
            bytes.push(5);
            bytes.extend_from_slice(&(checks.len() as u64).to_be_bytes());
            for check in checks {
                text(&mut bytes, &check.name);
                bytes.push(check.status as u8);
                evidence(&mut bytes, &check.evidence);
            }
        }
        ReasoningPayload::Outcome { outcome, summary } => {
            bytes.push(6);
    ...
    fn a_complete_run_is_typed_ordered_and_hash_chained() {
        ...
        run.append(
            ...
            ReasoningPayload::Plan {
                hypothesis: "bug is temporal".into(),
                steps: vec!["test".into()],
            },
        )
        .unwrap();
        run.append(
            ...
            ReasoningPayload::Attempt {
                ...
                actions: vec!["edit temporal.rs".into()],
            },
        )
        .unwrap();
        run.append(
        ...
        run.append(
            ...
            ReasoningPayload::Decision {
                ...
                rationale: "candidate is ready".into(),
            },
        )
        .unwrap();
        run.append(
        ...
        run.append(
            ...
            ReasoningPayload::Outcome {
                ...
                summary: "acceptance met".into(),
            },
        )
        .unwrap();
        assert!(run.is_complete());
        assert_eq!(
            run.events()[1].previous_digest.as_deref(),
            Some(run.events()[0].digest.as_str())
        );
