...
use vyrm_core::{
    ...
    ScopeId, VectorValue, DATA_RUNTIME_CONTRACT_VERSION,
};

pub const TURBOQUANT_SEGMENT_FORMAT_VERSION: u16 = 1;
const TURBOQUANT_SEGMENT_MAGIC: &[u8; 8] = b"VYRTQ001";
...
impl TurboQuantSegment {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self> {
        ...
        let header_len =
            usize::try_from(header_len).map_err(|_| vyrm_core::Error::InvalidRuntime {
                reason: "TurboQuant header length overflows this host".into(),
            })?;
        let payload_len =
            usize::try_from(payload_len).map_err(|_| vyrm_core::Error::InvalidRuntime {
        ...
        let advertised =
            std::str::from_utf8(&bytes[22..86]).map_err(|_| vyrm_core::Error::InvalidRuntime {
...
fn artifact_digest(header: &[u8], payload: &[u8]) -> String {
    let mut bytes = b"vyrm-turboquant-segment-v1\0".to_vec();
...
fn encode_artifact(header: &[u8], payload: &[u8], artifact_digest: &str) -> Result<Vec<u8>> {
    let header_len = u32::try_from(header.len()).map_err(|_| vyrm_core::Error::InvalidRuntime {
        reason: "TurboQuant header exceeds u32".into(),
    })?;
    let payload_len =
        u64::try_from(payload.len()).map_err(|_| vyrm_core::Error::InvalidRuntime {
...
mod tests {
    ...
    use vyrm_core::{ReadStamp, RuntimeValue, RuntimeVector, VectorValue};
