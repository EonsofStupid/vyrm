...
use rrd_engine::{Registry, Verification, VERIFICATION_TTL_MS};
use rrd_store::Store;
...
fn verification_is_a_bitemporal_claim_that_expires_and_makes_noise() {
    ...
    let claim = registry
        .record_verification(
            ...
            "codex-cli",
            1_000,
            "checked v3.2 against AGENTS.md docs",
            "operator:test",
        )
        .unwrap();
    ...
    assert!(matches!(
        registry
            .verification(&store, adapter, 1_000 + VERIFICATION_TTL_MS - 1)
            .unwrap(),
        Verification::Current { .. }
    ));
    assert_eq!(
        registry
            .verification(&store, adapter, 1_000 + VERIFICATION_TTL_MS)
            .unwrap(),
    ...
    assert_eq!(
        registry
            .verification(&store, adapter, 1_000 + 34 * day)
            .unwrap(),
    ...
    assert!(matches!(
        registry
            .verification(&store, adapter, 1_000 + 35 * day)
            .unwrap(),
        Verification::Current { .. }
    ));
}
...
fn preflight_surfaces_the_expired_adapter_at_a_fixed_instant() {
    ...
    registry
        .record_verification(
            ...
            "claude-code",
            1_000,
            "hooks reference checked",
            "operator:test",
        )
        .unwrap();

    let reader = rrd_core::Reader::new("test:preflight").unwrap();
    let day = 24 * 60 * 60 * 1000;

    // Inside the interval: no noise.
    let quiet = rrd_engine::preflight(
        &store,
        dir.path(),
        Some("claude-code"),
        &reader,
    ...
    let noisy = rrd_engine::preflight(
        &store,
        dir.path(),
        Some("claude-code"),
        &reader,
    ...
    assert!(
        noisy
            .warnings
            .iter()
            .any(|w| w.contains("unverified for 13 day(s)")),
