...
pub const WORKFLOW_FILE: &str = ".rrflow/workflows.toml";
...
mod tests {
    ...
    fn binding(root: &Path, id: &str) -> InstanceBinding {
        InstanceBinding {
            manifest: crate::runtime::InstanceManifest::dedicated(id).unwrap(),
    ...
    fn write_manifest(root: &Path, extra: &str) {
        std::fs::create_dir_all(root.join(".rrflow")).unwrap();
        std::fs::write(
