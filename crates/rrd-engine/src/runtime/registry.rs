...
use rrd_core::{Claim, ClaimReader, Millis, Predicate, Producer, Subject};
...
impl Harness {
    ...
    pub fn degradations(&self) -> Vec<String> {
        ...
        out.push(format!(
            "{}: no hook lifecycle — recall is on-demand (MCP via rrflow-mcp), not injected before reasoning",
            self.name
        ));
        if !self.mcp_client {
            out.push(format!(
                "{}: no MCP client — CLI-only integration",
                self.name
            ));
        }
        out
...
impl Registry {
    ...
    pub fn record_verification<E: Engine>(
        ...
        let harness = self.get(name).ok_or_else(|| {
            rrd_store::Error::Substrate(format!("no harness named {name:?} in the registry"))
        })?;
    ...
    fn every_embedded_row_parses_and_axes_are_populated() {
        ...
        for harness in registry.all() {
            assert!(
                ...
                "{} declares no billing mode",
                harness.name
            );
            assert!(
                !harness.providers.is_empty(),
