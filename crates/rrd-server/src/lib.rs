...
impl RrdService<vyrm_store::PersistentEngine> {
    #[allow(clippy::too_many_arguments)]
    pub fn create_instance_backup(
        ...
        let existing = self.engine.control_record(&key)?;
        ...
        let catalogue = vyrm_store::verify_backup_catalogue(&root)?;
        ...
        let entry = match matching.first() {
            ...
            None => vyrm_store::create_logical_backup(
                &self.engine,
                &root,
                &state.storage_label,
    ...
    pub fn list_instance_backups(
        ...
        let catalogue = if request.verify_archives {
            vyrm_store::verify_backup_catalogue(&self.backup_root())?
        } else {
            vyrm_store::load_backup_catalogue(&self.backup_root())?
        };
    ...
    pub fn restore_instance_backup(
        ...
        let catalogue = vyrm_store::verify_backup_catalogue(&self.backup_root())?;
        ...
        let (inventory, reopened, recovered) = if target.exists() {
            let restored = vyrm_store::PersistentEngine::open(&target)?;
            ...
            let report = vyrm_store::restore_catalogued_backup(
                &self.backup_root(),
...
fn public_backup_catalogue(
    catalogue: &vyrm_store::BackupCatalogue,
...
fn public_backup(entry: &vyrm_store::BackupEntry) -> InstanceBackupSnapshot {
...
fn public_backup_coverage(
    value: vyrm_store::BackupCoverage,
) -> rrd_contract::BackupCoverageSnapshot {
    match value {
        vyrm_store::BackupCoverage::Included => rrd_contract::BackupCoverageSnapshot::Included,
        vyrm_store::BackupCoverage::ReferencedOnly => {
            rrd_contract::BackupCoverageSnapshot::ReferencedOnly
        }
        vyrm_store::BackupCoverage::RebuildRequired => {
            rrd_contract::BackupCoverageSnapshot::RebuildRequired
        }
        vyrm_store::BackupCoverage::Excluded => rrd_contract::BackupCoverageSnapshot::Excluded,
    }
}

fn public_archive(inventory: &vyrm_store::LogicalArchiveInventory) -> LogicalArchiveSnapshot {
