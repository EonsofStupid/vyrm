...
const HOLD_AFTER_EFFECT_ENV: &str = "RRD_ESTATE_TEST_HOLD_AFTER_EFFECT_FILE";
const HOLD_AFTER_STEP_ENV: &str = "RRD_ESTATE_TEST_HOLD_AFTER_STEP_FILE";
...
fn main() {
    if let Err(error) = run() {
        eprintln!("rrd-estate-controller: {error}");
        std::process::exit(2);
    }
}
...
fn parse_args(arguments: impl Iterator<Item = String>) -> Result<Args, String> {
    ...
    while let Some(argument) = arguments.next() {
        match argument.as_str() {
            ...
            "--catalog" => {
                catalog = Some(PathBuf::from(required(&mut arguments, "--catalog")?));
            }
            "--estate" => {
                estate = Some(
...
mod tests {
    ...
    #[test]
    fn requires_the_complete_controller_identity() {
