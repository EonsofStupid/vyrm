...
//!
//! ```text
//! cargo run --release -p rrd-graph --example route_vs_scan -- <repo-root> <query>...
//! ```

use rrd_graph::attune_and_index;

fn main() {
    let mut args = std::env::args().skip(1);
    let root = args
        .next()
        .expect("usage: route_vs_scan <repo-root> <query>...");
    ...
    let (profile, index) = attune_and_index(std::path::Path::new(&root)).expect("attune and index");
