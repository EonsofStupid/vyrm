...
//!
//! ```text
//! cargo run --release -p rrd-graph --example route_persisted -- <store-path> <repo-root> <query>...
//! ```
...
fn main() {
    ...
    let store_path = args
        .next()
        .expect("usage: route_persisted <store-path> <repo-root> <query>...");
    let root = args
        .next()
        .expect("usage: route_persisted <store-path> <repo-root> <query>...");
    ...
    store
        .put_projection(&projection, &bytes)
    ...
    println!(
        "refresh after load:  {refresh_ms} ms ({})",
        refresh.render()
    );
    println!(
