...
fn a_refresh_with_no_changes_reads_nothing_and_reports_a_noop() {
    ...
    assert_eq!(
        ...
        "unchanged files were read anyway"
    );
    assert_eq!(
        index.generation(),
        generation,
...
fn only_the_changed_file_is_re_extracted() {
    ...
    assert_eq!(
        index.route("alpha_two", 5).len(),
        1,
        "new symbol not routable"
    );
}
...
fn a_touched_but_unmodified_file_is_read_once_and_then_skipped() {
    ...
    assert_eq!(
        second.skipped_unread, 1,
        "stats were not updated after the read"
    );
}
...
fn additions_and_removals_are_tracked() {
    ...
    assert!(
        index.route("gamma", 5).is_empty(),
        "a removed symbol stayed routable"
    );
}
...
fn grounding_detects_a_stale_projection_rather_than_repairing_it() {
    ...
    assert!(
        !grounding.agreed,
        "a stale projection was reported as grounded"
    );
