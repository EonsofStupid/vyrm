...
fn centrality_never_routes_a_file_unrelated_to_the_query() {
    ...
    assert_eq!(
        routed.len(),
        1,
        "an unrelated file was routed on centrality alone"
    );
...
fn budget_fill_always_includes_the_top_ranked_file() {
    ...
    assert_eq!(
        routed.len(),
        1,
        "the top-ranked file must be routed even over budget"
    );
...
fn budget_fill_skips_an_oversized_file_and_takes_a_smaller_one() {
    ...
    let dir = project(&[
        ...
        (
            "src/light.rs",
            &padded("pub fn l() { crate::def::target_name(); }", 50),
        ),
    ]);
    let index = indexed(&dir);
    ...
    assert!(
        ranked[1].path.ends_with("src/heavy.rs"),
        "heavy must rank above light"
    );
    ...
    assert_eq!(
        paths.len(),
        2,
        "expected def.rs and light.rs, got {paths:?}"
    );
    assert!(paths[0].ends_with("src/def.rs"));
    assert!(
        paths[1].ends_with("src/light.rs"),
        "the smaller file was not back-filled"
    );
