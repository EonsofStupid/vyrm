...
fn a_module_file_is_the_definition_site_of_its_stem() {
    ...
    let dir = project(&[
        (
            ...
            "export function translateQuant(): string { return \"\"; }",
        ),
        (
            "src/caller.ts",
            "import { translateQuant } from \"./utils/terminology\";\nterminology;",
        ),
    ]);
...
fn an_entry_file_declares_its_directory_not_its_stem() {
    ...
    assert!(
        index
            .route("index", 5)
            .iter()
            .all(|r| r.justification.defines.is_empty()),
...
fn a_compound_extension_declares_the_first_stem_segment() {
    let dir = project(&[(
        "src/runes/hardwareState.svelte.ts",
...
fn a_real_declaration_is_not_shadowed_by_the_synthesized_line() {
    ...
    assert_eq!(
        routed[0].justification.defines.len(),
        1,
        "duplicate definition injected"
    );
}
